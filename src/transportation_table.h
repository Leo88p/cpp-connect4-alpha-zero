...
namespace Connect4 {
    ...
    struct TTEntry {
        uint64_t hash_key = 0;
        int8_t score = 0;
        uint8_t depth = 0; // Now stores 'depth_remaining'
    ...
    public:
        ...
        void store(uint64_t hash_key, int8_t score, uint8_t depth, EntryType type, int8_t best_move) {
            ...
            // Overwrite if the slot is empty OR if the new search is deeper/equal to the stored one.
            // This ensures we don't overwrite a deep search with a shallow one for the same position.
            if (entry.hash_key == 0 || depth >= entry.depth) {
